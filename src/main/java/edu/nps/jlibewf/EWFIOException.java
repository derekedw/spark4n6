...
public final class EWFIOException extends IOException {
  ...
  private long address;
    ...
    /**
   * Constructs an Exception event containing the file ands address that sourced the exception.
   * @param file the file from which the exception originated
   * @param address the address from which the exception originated
   */
  protected EWFIOException(String message, Path file, long address) {
    super(message);
    this.file = file;
  ...
  public String getMessage() {
      return super.getMessage() + " at file " + file.toUri().toASCIIString() + " file address "
